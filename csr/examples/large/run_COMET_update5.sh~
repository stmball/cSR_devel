...
# ----------------------------------------------
# Takes data from updates 1-4 in EndNote XML format
# with MN as negative labels and Y as positive.
# Applies the model on the candidate in update 5
# and exports the ordered file to sorted/out.xml
# This is the setting that was used in the 2019
# update
# Note that running this script will take a fair
# amount of time. To check that everything works
# as intended, it may be a good idea to first run
# the smaller example version: 'run_COMET.sh'
# ===============================================
...
if [ $? -eq 0 ]; then
    python -m examples.preprocess_data \
	   --format endnote_xml \
	   --train_N_paths \
...
if [ $? -eq 0 ]; then
    python -m examples.sort_endnote_xml \
