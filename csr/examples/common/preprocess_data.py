...
parser.add_argument(
    "--train_N_paths", nargs="+", required=True, help="Negative examples"
)
parser.add_argument(
    "--train_Y_paths", nargs="+", required=True, help="Positive examples"
)
parser.add_argument(
    "--test_paths", nargs="+", required=True, help="Data to apply the trained model on"
)
parser.add_argument("--out_path", required=True, help="Path to save the data to")
parser.add_argument(
    "--format",
    choices=["bibreview", "endnote_xml", "embase_xml", "RIS", "pmids", "tsv", "csv"],
    required=True,
    help="Input format",
)
...
noop = lambda x: x
tokenize = Tokenizer(
    sent_tokenize=noop_sent_tokenize,
...
def assert_no_duplicates(data):
    ...
    for desc, row in zip(descs, data):
        if not desc in desc_to_ref:
            desc_to_ref[desc] = []
        desc_to_ref[desc].append(row)
    duplicates = [
        (desc, count) for desc, count in collections.Counter(descs).items() if count > 1
    ]
    assert duplicates == [], "%i/%i duplicates in set: \n%s" % (
        ...
        "\n".join(
            [
                ...
                + "\n".join(
                    [
                        str((ref.journal, ref.year, ref.title))
                        for ref in desc_to_ref[desc]
                    ]
                )
...
def assert_is_subset(data_sub, data_sup):
    ...
    if not is_subset:
        print(
            "%i references from other sources:\n%s"
...
for path in args.train_Y_paths:
    ...
    # If Path is a directory, process all files in it
    if path.is_dir():
        for sub_path in path.glob("**/*"):
            records = import_handler(open(sub_path, "rb").read())
            records.delete_if(seen_already(prev_descs))
            if not train_Y:
                train_Y = records
            else:
                train_Y.merge(records)
    else:
...
if not train_Y:
    raise ValueError("No positive training data found")
