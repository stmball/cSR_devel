...
class SharedDataView:
    ...
    def get_data(self, indices):
        ...
        # number of data point in resulting data matrix
        n = len(indices)
        # number of features in resulting data matrix
        m = self._X_t.shape[1]
...
#        print(self._indices[Split.TRAIN][Split.TEST])
    ...
    def calculate_f1(self, split):
        return self.calculate_ranking_score(split, metrics.f1_score)
...
#                trainer.print_most_important_features(X_pos)
    ...
    def run(self, display = None, eval_metrics = ['AUC'], verbosity = 0):
        ...
        eval_metric_func = {
            'AUC': Trainer.calculate_auc,
            'AP':  Trainer.calculate_ap
            'FM':  Trainer.calculate_f1
        }
        unimplemented_metrics = set(eval_metrics) - set(eval_metric_func.keys())
        if unimplemented_metrics:
            raise ValueError('Unrecognized metric(s): %s' % ', '.join(unimplemented_metrics))
...
#                unseen_X_t = trainer._X_t[unseen_i, :]
                ...
                f1=metrics.f1_score(numpy.array(seen_labels + unseen_labels) == 'Y',
                                                      labels, 
							average='weighted')
                f1_per_trainer.append(f1)
...
#                STATUS[5]('Intertopic training data used: %i' % (len(train_ind)))
            ...
            STATUS[5]('Mean FM:  %0.3f' % numpy.mean(f1_per_trainer))
